...
/*      */       }
      ...
      break;
            ...
            case 30008006:
                ...
                case 30018006: {
                int boost = applyto.getBuffedValue(MapleBuffStat.BOOSTER) == null ? 0 : applyto.getBuffedValue(MapleBuffStat.BOOSTER);
                localstatups = new EnumMap<>(MapleBuffStat.class);
                int speedBonus = 0;
                if(applyto.getBuffedValue(MapleBuffStat.C_SPEEDINFUSION) != null){
                    speedBonus -= applyto.getBuffedValue(MapleBuffStat.C_SPEEDINFUSION);
                }
                if(boost < 0){
                    localstatups.put(MapleBuffStat.BOOSTER, -3 + speedBonus);
                } else {
                    localstatups.put(MapleBuffStat.BOOSTER, -1 + speedBonus);
                }
                localstatups.put(MapleBuffStat.UNK_6_20, 1);
                break;
            }
                case 5121009:
                case 15111005: {
                ...
                if(boost < 0){
                    localstatups.put(MapleBuffStat.BOOSTER, -4 + speedBonus);
                } else {
                    localstatups.put(MapleBuffStat.BOOSTER, -2 + speedBonus);
                }
                localstatups.put(MapleBuffStat.UNK_6_20, 1);
                break;
            }
            ...
            case 5001005: { //dash
                int buffID = 2259100 + applyto.getTotalSkillLevel(SkillFactory.getSkill(5001005));
                MapleItemInformationProvider.getInstance().getItemEffect(buffID).applyTo(applyto);
                break;
            }
            ...
            default:
                ...
                if (localstatups.containsKey(MapleBuffStat.BOOSTER)) {
                    int speedIncrease = -2;
                    if(applyfrom.getBuffedValue(MapleBuffStat.C_SPEEDINFUSION) != null){
                        speedIncrease = -3;
                    }
                    int sourceID = applyto.getBuffSource(MapleBuffStat.BOOSTER);
                    if (sourceID % 10000 == 8006) { //dSI is active
                        speedIncrease = -3;
                    } else if (sourceID == 15111005 || sourceID == 5121009) {
                        speedIncrease = -4;
                    }
                    localstatups.remove(MapleBuffStat.BOOSTER);
                    localstatups.put(MapleBuffStat.BOOSTER, speedIncrease);
                    localstatups.put(MapleBuffStat.ACASH_RATE, 1);
                }
