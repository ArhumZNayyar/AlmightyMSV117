...
public class MapleMonster extends AbstractLoadedMapleLife {
 ...
 public final ChangeableStats getChangedStats() {
    ...
    public final void hellChangeLevel(final double newLevel, final double hpBuff, final double bossHpBuff, final double expMulti, final double evaMulti) { //Custom hell
        //To define a non-changeable mob, give it the <int name="changeableMob" value="1"/> property in its XML, in the "info" node.
        if (stats.isNotChangeable()) {
            return;
        }
        
        this.ostats = new ChangeableStats(stats, newLevel, hpBuff, bossHpBuff, expMulti, evaMulti);
        this.hp = ostats.getHp();
        this.mp = ostats.getMp();
    }
    
    /*   public final void TormentMode(final double newHP) { //Custom hell
        //To define a non-changeable mob, give it the <int name="changeableMob" value="1"/> property in its XML, in the "info" node.
        if (stats.isNotChangeable()) {
            return;
        }
        
        this.ostats = new ChangeableStats(stats, newHP);
        this.hp = ostats.getHp();
        this.mp = ostats.getMp();
    }
    */
  ...
  public final void damage(final MapleCharacter from, final long damage, final boolean updateAttackTime, final int lastSkill) {
    ...
    private final void giveExpToCharacter(final MapleCharacter attacker, int exp, final boolean highestDamage, final int numExpSharers, final byte pty, final byte Class_Bonus_EXP_PERCENT, final byte Premium_Bonus_EXP_PERCENT, final int lastskillID) {
        ...
        if (exp > 0) {
            ...
            if (holySymbol != null) {
                exp *= 1.4 + (holySymbol.doubleValue() / 100.0);
            }
            if (holySymbol != null && attacker.getReborns() >= 8) {
                exp *= 0.85;
            }
            if (holySymbol != null && attacker.getReborns() >= 14) {
                exp *= 0.71;
            }
            if (holySymbol != null && attacker.getReborns() >= 20) {
                exp *= 0.40;
            }
              if (holySymbol != null && attacker.getReborns() >= 30) {
                exp *= 0.33;
            }
              if (holySymbol != null && attacker.getReborns() >= 40) {
                exp *= 0.28;
            }
            ...
            int Equipment_Bonus_EXP = (int) ((exp / 33.2) * attacker.getStat().equipmentBonusExp);
    ...
    public final void handleSteal(MapleCharacter chr) {
        ...
        if (level > 0 && !getStats().isBoss() && stolen == -1 && steal.getEffect(level).makeChanceResult()) {
            ...
            for (MonsterDropEntry d : dropEntry) { //set to 4x rate atm, 40% chance + 10x
                if (d.itemId > 0 && d.questid == 0 && d.itemId / 10000 != 238 && Randomizer.nextInt(999999) < (int) (10 * d.chance * chServerrate * chr.getDropMod() * (chr.getStat().dropBuff / 100.0 + 4) * (showdown / 100.0))) { //kinda op
