...
import static handling.world.PartyOperation.CHANGE_LEADER;
import static handling.world.PartyOperation.CHANGE_LEADER_DC;
import static handling.world.PartyOperation.DISBAND;
import static handling.world.PartyOperation.EXPEL;
import static handling.world.PartyOperation.JOIN;
import static handling.world.PartyOperation.LEAVE;
import static handling.world.PartyOperation.LOG_ONOFF;
import static handling.world.PartyOperation.SILENT_UPDATE;
...
/*      */   }
/*      */  public static byte[] updatePlayerStatsL(Map<MapleStat, Long> mystats, boolean itemReaction, MapleCharacter chr) {
...
/* 2705 */         mplew.write(((Long)statupdate.getValue()).byteValue());
/* 2706 */         break;
                case JOB:
                case STR:
                case DEX:
                ...
                case LUK:
                case AVAILABLEAP:
/* 2713 */         mplew.writeShort(((Long)statupdate.getValue()).shortValue());
/* 2714 */         break;
/*      */        case AVAILABLESP:
...
/* 2727 */         break;
/*      */       case TRAIT_LIMIT:
/* 2729 */         mplew.writeInt(((Long)statupdate.getValue()).intValue());
/* 2730 */         mplew.writeInt(((Long)statupdate.getValue()).intValue());
/* 2731 */         mplew.writeInt(((Long)statupdate.getValue()).intValue());
/* 2732 */         break;
/*      */        case PET:
/* 2734 */         mplew.writeLong(((Long)statupdate.getValue()).intValue());
/* 2735 */         mplew.writeLong(((Long)statupdate.getValue()).intValue());
/* 2736 */         mplew.writeLong(((Long)statupdate.getValue()).intValue());
/* 2737 */         break;
/*      */       default:
/* 2739 */         mplew.writeInt(((Long)statupdate.getValue()).intValue());
/*      */       }
/*      */     }
/*      */ 
...
/*final OutPacket op = new OutPacket();
        op.EncodeHeader(SendOpcode.BroadcastMsg.getValue());
        op.Encode1(type);
        switch (type) {
//excluded others
            case 7:
                op.EncodeStr(message);
                op.Encode4(npc);
                break;
        }
        return op.getPacket();
 ...
 public static class ExpeditionPacket {
    ...
    public static class PartyPacket {
        ...
        public static byte[] partyRequestInvite(MapleCharacter from) {
            ...
            mplew.writeMapleAsciiString(from.getName());
            //mplew.writeShort(from.getJob());
            //mplew.writeShort(10);//unk
            //mplew.write(from.getJob());
            mplew.writeInt(from.getLevel());
            mplew.writeInt(from.getJob());
            mplew.writeInt(0);
...
/*      */ 
        ...
        public static byte[] giveBuff(int buffid, int bufflength, Map<MapleBuffStat, Integer> statups, MapleStatEffect effect) {
            MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
            List<Pair<MapleBuffStat, Integer>> newstatups = PacketHelper.sortBuffStats(statups);
            ...
            for (Pair<MapleBuffStat, Integer> stat : newstatups) {
                if ((stat.left == MapleBuffStat.YELLOW_AURA) || (stat.left == MapleBuffStat.BLUE_AURA) || (stat.left == MapleBuffStat.DARK_AURA)) {
                    isAura = true;
                }
                if (((MapleBuffStat) stat.left).canStack()) {
                    if (!stacked) {
                        mplew.writeZeroBytes(3);
                        stacked = true;
                    }
                    mplew.writeInt(1);
                    mplew.writeInt(buffid);
                    mplew.writeLong(((Integer) stat.right).longValue());
                } else {
                    if (statups.containsKey(MapleBuffStat.BODY_BOOST) || statups.containsKey(MapleBuffStat.JUDGMENT_DRAW)) {
                        mplew.writeInt(0);
                    }
                    if (stat.left == MapleBuffStat.SPIRIT_SURGE) {
                        mplew.writeInt(((Integer) stat.right).intValue());
                    } else {
                        mplew.writeShort(((Integer) stat.right).intValue());
                    }
                    mplew.writeInt(buffid);
                }
                mplew.writeInt(bufflength);
...
/*      */    // }
   ...
   public static byte[] giveForeignBuff(int cid, Map<MapleBuffStat, Integer> statups, MapleStatEffect effect) {
            ...
            for (Entry<MapleBuffStat, Integer> statup : statups.entrySet()) {
                if (statup.getKey() == MapleBuffStat.SHADOWPARTNER || statup.getKey() == MapleBuffStat.MECH_CHANGE
                        || statup.getKey() == MapleBuffStat.DARK_AURA || statup.getKey() == MapleBuffStat.YELLOW_AURA
                        || statup.getKey() == MapleBuffStat.BLUE_AURA || statup.getKey() == MapleBuffStat.GIANT_POTION
                        || statup.getKey() == MapleBuffStat.SPIRIT_LINK || statup.getKey() == MapleBuffStat.PYRAMID_PQ
                        || statup.getKey() == MapleBuffStat.WK_CHARGE || statup.getKey() == MapleBuffStat.SPIRIT_SURGE
                        || statup.getKey() == MapleBuffStat.MORPH || statup.getKey() == MapleBuffStat.WATER_SHIELD
