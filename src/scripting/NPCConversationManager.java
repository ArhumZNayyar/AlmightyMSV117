...
public class NPCConversationManager extends AbstractPlayerInteraction {
    ...
    public final void clearSkillPoint() {
...
//public void teachSkill(int id, int skillevel, byte masterlevel, long expiration) {
 ...
 //}
   ...
   public void increaseGuildCapacity(boolean trueMax) {
    ...
    public void killPlayer() {
         Map<MapleStat, Integer> statup = new EnumMap<MapleStat, Integer>(MapleStat.class);
                ...
                c.getPlayer().dropMessage(6, "Rekt");
    }
    ...
    public int setAndroid(int ticket, int args) {
        if (!haveItem(ticket)) {
            return -1;
        }
        gainItem(ticket, (short) -1);
   ...
   public static int editEquipById(MapleCharacter chr, int max, int itemid, String stat, int newval) {
    ...
    public final void resetSkills(int sp ) {
            c.getPlayer().resetSP(sp);
            c.getPlayer().dropMessage(6, "Your skills have been reset.");
        }
        ...
        CloneTimer.getInstance().schedule(new Runnable() {
            ...
            public void run() {
                if (chr == null || player == null) {
                    ...
                    chr.getMap().startExtendedMapEffect("You may now kiss the bride, " + player.getName() + "!", 5122000);
                    if (chr.getGuildId() > 0) {
...
byte ret = (byte) getPlayer().getInventory(MapleInventoryType.EQUIP).addItem(it);
c.getSession().write(CWvsContext.InventoryPacket.addInventorySlot(MapleInventoryType.EQUIP, it));
...
public int getCodeLog(String codeid) {
    return getPlayer().getCodeLog(codeid);
}
public void setCodeLog(String codeid) {
    getPlayer().setCodeLog(codeid);
}
public int getBountyLog(String bountyid) {
    return getPlayer().getBountyLog(bountyid);
}
public void setBountyLog(String bountyid) {
    getPlayer().setBountyLog(bountyid);
}

public int getScavengerLog(String scavid) {
    return getPlayer().getScavengerLog(scavid);
}
public void setScavengerLog(String scavid) {
    getPlayer().setScavLog(scavid);
}

public long checkCodes(String code) {
    long ret = 0;
    	try {
    		Connection con = DatabaseConnection.getConnection();
    		PreparedStatement ps = con.prepareStatement("SELECT code FROM codes WHERE id = ?");
    		ps.setLong(1, id);
    		ResultSet rs = ps.executeQuery();
    		if (rs.next()) {
    			ret = rs.getLong("code");
...
public int getBossLog(String bossid) {
   ...
   public void reloadMap() {
          MapleMap oldMap = c.getPlayer().getMap();
                MapleMap newMap = c.getChannelServer().getMapFactory().getMap(id);
                for (MapleCharacter ch : oldMap.getCharacters()) {
                    ch.changeMap(newMap);
                }
                oldMap = null;
                c.getPlayer().getMap().respawn(pendingDisposal);
...
public void sendSimpler(String text, String... selections) {
   ...
   public void sendSimple(String text, String... selections) { 
    ...
    if (match.find()) { 
        ...
        while (match.find() && i++ < selections.length) { 
            sb.append(text.substring(lme, match.start())); 
             
            sb.append("#L" + i + "#" + selections[Integer.parseInt(match.group().replace("$", "").replace("{", "").replace("}", ""))] + "#l\r\n"); 
            lme = match.end(); 
        } 
    } else { 
    ...
    sendSimple(sb.length() > 0 ? sb.toString() : text); 
}  
   ...
   public int getPandoraItem(int id) {
    ...
    public long getMesosBank() {
        return getPlayer().getMesosBank();
    }  
//ALTER TABLE `characters` ADD COLUMN `mesosbank` BIGINT NOT NULL DEFAULT 0;  
      
    public void doNameChange(String name) {//Simpleton
        c.getPlayer().doNameChange(name);
    }
    
    public boolean ifNameExist(String name) {//Simpleton
        if (c.getPlayer().ifNameExist(name))
            return true;
        return false;
    }
    
    public boolean hasChangedName(String name) {//Simpleton
     ...
     public String etcList(MapleCharacter player) {
        StringBuilder str = new StringBuilder();
        MapleInventory etc = c.getPlayer().getInventory(MapleInventoryType.ETC);
        for (Item item : etc.list()) {
            str.append("#L");
            str.append(item.getPosition());
            str.append("##v");
