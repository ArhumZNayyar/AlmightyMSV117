...
public class InventoryHandler {
    ...
    public static final void UseSummonBag(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        if (toUse != null && toUse.getQuantity() >= 1 && toUse.getItemId() == itemId && (c.getPlayer().getMapId() < 910000021 || c.getPlayer().getMapId() > 910000022)) {
    ...
    public static final void UseCashItem(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        switch (itemId) {
            case 5155000: {
                c.getPlayer().dropMessage(5, "Wowe, you got human ears now.");
            ...
            case 5570000: { // Vicious Hammer
                ...
                if (item != null) {
                    if (GameConstants.canHammer(item.getItemId()) && MapleItemInformationProvider.getInstance().getSlots(item.getItemId()) > 0 && item.getViciousHammer() < 2) {
                        ...
                        c.getPlayer().dropMessage(5, "You may not use it on this item :c");
...
c.getPlayer().dropMessage(1, "You may not use this until level 100."); //gunna get flamed for this level req
} else {
...
if (eq.getState() >= 17) {
eq.renewPotential(3); //fk da 4th and 5th lines
c.getPlayer().getMap().broadcastMessage(CField.showPotentialReset(false, c.getPlayer().getId(), true, itemId));
c.getSession().write(InventoryPacket.scrolledItem(toUse, item, false, true));
c.getPlayer().forceReAddItem_NoUpdate(item, MapleInventoryType.EQUIP);
MapleInventoryManipulator.addById(c, 2430481, (short) 1, "Cube" + " on " + FileoutputUtil.CurrentReadable_Date());
...
c.getPlayer().dropMessage(5, "This item is unchangeable."); //lol non-cubed item hurr durrr
            }
        } 
else {
c.getPlayer().getMap().broadcastMessage(CField.showPotentialReset(false, c.getPlayer().getId(), false, itemId));
      ...
      case 5090100: // Wedding Invitation Card
            ...
            case 5501002: { //expiry mount
                ...
                final long toAdd = (itemId == 5501001 ? 30 : 60) * 24 * 199 * 999 * 1000L;
                final long expire = c.getPlayer().getSkillExpiry(skil);
                if (expire < System.currentTimeMillis() || (long) (expire + toAdd) >= System.currentTimeMillis() + (365 * 2665 * 64 * 60 * 1000L)) {
                    break;
                }
                ...
                if (MapleItemInformationProvider.getInstance().isPickupBlocked(mapitem.getItemId())) {
                    ...
                    if(!"".equals(mapitem.getInstanceOwner())){
                        if(!mapitem.getInstanceOwner().equals(chr.getName())){
                            chr.dropMessage(6, "You cannot pick up this drop.  It is not instanced for you.");
                            c.getSession().write(CWvsContext.enableActions());
                        } else {
                    ...
                    if(chr.isGM())
                        System.out.println("[GM ONLY DEBUG] Else case in pickup_player, invenfull.");
                    c.getSession().write(InventoryPacket.getInventoryFull());
                    c.getSession().write(InventoryPacket.getShowInventoryFull());
                    c.getSession().write(CWvsContext.enableActions());
                }
  ...
  public static final void Pickup_Pet(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
    ...
    public static final boolean UseTeleRock(LittleEndianAccessor slea, MapleClient c, int itemId) {
     ...
     public static final void useInnerCirculator(LittleEndianAccessor slea, MapleClient c) {
      ...
      public static int get_gender_from_id(int itemId) { 
        int result; // eax@4 

        if (itemId / 1000000 != 1 && itemId / 10000 != 254) { 
            result = 2; 
        } else { 
      ...
      public static int is_correct_bodypart(int itemId, short dst, byte gender) { 
        int v3; // eax@1 
        int result; // eax@4 
        int v5; // eax@58 
         
        dst = (short) -dst; //dst of equipped inventory is lower than 0 
         
        v3 = get_gender_from_id(itemId); 
        if (gender == 2 || v3 == 2 || v3 == gender) { 
            switch (itemId / 10000) { 
                case 100: 
                    if (dst != 1 && dst != 1200) 
                        result = 0; 
                ...
                case 181: 
                    switch (itemId) { 
                        case 1812001: 
                            if (dst == 22 || dst == 33 || dst == 41) 
                                result = 1; 
                            else 
                                result = 0; 
                            break; 
                        case 1812000: 
                            if (dst == 23 || dst == 34 || dst == 42) 
                                result = 1; 
                            else 
                                result = 0; 
                            break; 
                        ...
                        case 1812003: 
                            ...
                            break; 
                        case 1812004: 
                            if (dst == 26 || dst == 35 || dst == 43) 
                                result = 1; 
                            else 
                                result = 0; 
                            break; 
                        case 1812005: 
                            if (dst == 27 || dst == 36 || dst == 44) 
                                result = 1; 
                            else 
                                result = 0; 
                            break; 
                        case 1812006: 
                            if (dst == 28 || dst == 37 || dst == 45) 
                                result = 1; 
                            else 
                                result = 0; 
                            break; 
                        case 1812007: 
                            if (dst == 46 || dst == 47 || dst == 48) 
                                result = 1; 
                            else 
                                result = 0; 
                            break; 
                        case 1812008: 
                            if (dst == 57 || dst == 58 || dst == 59) 
                                result = 1; 
                            else 
                                result = 0; 
                            break; 
                ...
                case 182: 
                    ...
                    break; 
                case 183: 
                    if (dst == 29 || dst == 32 || dst == 40) 
                        result = 1; 
                    else 
                        result = 0; 
                    break; 
                case 120: 
                    if (dst == 5000 || dst == 5001 || dst == 5002) 
                        result = 1; 
                    else 
                        result = 0; 
                    break; 
                default: 
                    if (!GameConstants.isEquip(itemId) && itemId / 100000 != 16 && itemId / 100000 != 17) 
                        result = 0; 
                    else 
                        result = dst == 11 ? 1 : 0; 
                    break; 
