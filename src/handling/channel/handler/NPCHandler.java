...
public class NPCHandler {
    ...
    public static final void OpenPublicNpc(final LittleEndianAccessor slea, final MapleClient c) {
     ...
     public boolean isBank(int npcid) { 
        MapleDataProvider dataRoot = MapleDataProviderFactory.getDataProvider(new File("wz/Npc.wz")); 
        String npcId = ""; 
        if (npcid >= 1000000) { 
            npcId = "" + npcid; 
        } else if (npcid < 1000000 && npcid >= 100000) { 
            npcId = "0" + npcid; 
        } else if (npcid < 100000 && npcid >= 10000) { 
            npcId = "00" + npcid; 
        } else { 
            npcId = "000" + npcid; 
        } 
        MapleData npcData = dataRoot.getData(npcId + ".img"); 
        if (npcData.getChildByPath("info/trunkPut") != null) { 
            return true; 
        } else { 
            return false; 
        } 
