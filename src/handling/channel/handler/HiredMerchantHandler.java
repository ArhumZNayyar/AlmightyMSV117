...
public static void displayMerch(MapleClient c) {
        ...
        if (merch) {
            ...
            if (pack == null) {
                c.getSession().write(NPCPacket.getNPCTalk(9030000, (byte) 0, "You have no business here. Now go away, and I better not catch you having fun!", "00 00", (byte) 0));
                c.getPlayer().setConversation(0);
            } else if (pack.getItems().size() <= 0) { 
                if (!check(c.getPlayer(), pack)) {
                    c.getSession().write(PlayerShopPacket.merchItem_Message((byte) 0x21));
                    c.getSession().write(NPCPacket.getNPCTalk(9030000, (byte) 0, c.getPlayer().getName()+", You inventory can not hold the mesos you left behind, I suggest you put it into your bank via @NPC!", "00 00", (byte) 0));
                    return;
                }
                if (deletePackage(c.getPlayer().getAccountID(), pack.getPackageid(), c.getPlayer().getId())) {
                    ...
                    c.getSession().write(NPCPacket.getNPCTalk(9030000, (byte) 0, "So, you forgot to pick up your mesos? Don't worry, I saved it for you\r\nHere are your mesos: " + pack.getMesos()+". Thank you.", "00 00", (byte) 0));
                    c.getPlayer().setConversation(0);
                    deletePackage(c.getPlayer().getAccountID(), pack.getPackageid(), c.getPlayer().getId());
                    return;
                } else {
                    c.getPlayer().dropMessage(1, "Sol pls.");
                }
                c.getPlayer().setConversation(0);
            } else {
                if (!check(c.getPlayer(), pack)) {
                    c.getSession().write(PlayerShopPacket.merchItem_Message((byte) 0x21));
                    c.getSession().write(NPCPacket.getNPCTalk(9030000, (byte) 0, c.getPlayer().getName()+", your inventory can't hold all item's or meso's. Please make space first, then come back and talk to me!", "00 00", (byte) 0));
                    c.getPlayer().setConversation(0);
                ...
                c.getPlayer().gainMeso(pack.getMesos(), false);
                deletePackage(c.getPlayer().getAccountID(), pack.getPackageid(), c.getPlayer().getId());
                c.getSession().write(NPCPacket.getNPCTalk(9030000, (byte) 0, "I have saved your item's and mesos. You have gained "+pack.getMesos()+" meso. I better not catch you having fun!", "00 00", (byte) 0));
                c.getPlayer().setConversation(0);
            }
        }
        c.getSession().write(CWvsContext.enableActions());
