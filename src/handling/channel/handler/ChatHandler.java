...
public class ChatHandler {
    ...
    public static final void GeneralChat(final String text, final byte unk, final MapleClient c, final MapleCharacter chr) {
        if (text.length() > 0 && chr != null && chr.getMap() != null && !CommandProcessor.processCommand(c, text, chr.getBattle() == null ? CommandType.NORMAL : CommandType.POKEMON)) {
            ...
            if (chr.getCanTalk() || chr.isStaff()) {
                ...
                if (text.equalsIgnoreCase(c.getChannelServer().getServerName() + " makes me cum")) {
        ...
        switch (type) {
            ...
            case 4:
                if (chr.getParty() == null || chr.getParty().getExpeditionId() <= 0) {
                    c.getPlayer().dropMessage(5, "EXPED_DEBUG.");
                    break;
                }
                World.Party.expedChat(chr.getParty().getExpeditionId(), chattext, chr.getName());
                c.getPlayer().dropMessage(5, "EXPED_DEBUG: Sent");
                break;
        }
    }
  ...
  /*  public static final void SpouseChat(String spouseName, String msg, MapleClient c) {
        if (c.getPlayer().getMarriageId() >= 0) { // yay marriage
            if (c.getPlayer().getName().equalsIgnoreCase(spouseName)) { 
                if (c.getPlayer().getMarriageId() != null)
                    spouseName = c.getChannelServer().getPlayerStorage().getUserByID(c.getPlayer().
                else {
                    c.getPlayer().dropMessage(5, "You are not married or your spouse is currently offline.");
                    return;
                }
            }
            final int channel = World.Find.findChannel(spouseName);
            MapleCharacter spouse = ChannelServer.getInstance(channel).getPlayerStorage().getUserByName(spouseName);
            if (spouse != null) {
  ...
  public static final void Spouse_Chat(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        */
        ...
        if (channel > 0) {
            MapleCharacter spouseChar = ChannelServer.getInstance(channel).getPlayerStorage().getCharacterByName(spouse);
            if (spouseChar == null) {
                c.getPlayer().dropMessage(5, "You are not married or your spouse is offline.");
                c.announce(CWvsContext.enableActions());
                return;
 ...
 /*   public static final void Spouse_Chat(final LittleEndianAccessor slea, final MapleClient c) {
       final byte mode = slea.readByte();
       slea.readInt(); 
       final String partner = slea.readMapleAsciiString();
        int player = c.getPlayer().getMarriageId();
                if (c.getPlayer() == null || c.getPlayer().getMap() == null) {
                    return;
                }
                if (!c.getPlayer().getCanTalk()) {
                    c.getSession().write(CWvsContext.serverNotice(6, "You have been muted and are therefore unable to talk to your spouse."));
                    return;
                }
                c.getPlayer().getCheatTracker().checkMsg();
                final String recipient = slea.readMapleAsciiString();
                final String text = slea.readMapleAsciiString();
                final int ch = World.Find.findChannel(partner);
                if (ch > 0) {
                    MapleCharacter partner = ChannelServer.getInstance(ch).getPlayerStorage().getCharacterByName(partner);
                    if (player == null) {
                        return;
                    }
                    ...
                    if (c.isMonitored()) {
                        World.Broadcast.broadcastGMMessage(CWvsContext.serverNotice(6, c.getPlayer().getName() + " said to their spouse " + recipient + " : " + text));
