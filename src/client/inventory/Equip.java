...
public class Equip extends Item implements Serializable {
    ...
    public Item copy() {
        ...
        ret.evoLevel = evoLevel;
        return ret;
    }
    
     public byte getEvoLevel() {
        return evoLevel;
    }
    ...
    public void setEvoItemStats(short stats, short wa) {
        this.setStr((short)(this.getStr() + stats));
        this.setDex((short)(this.getDex() + stats));
        this.setInt((short)(this.getInt() + stats));
        this.setLuk((short)(this.getLuk() + stats));
        this.setWatk((short)(this.getWatk() + wa));
    }
    ...
    public void setPotential5(final int en) {
     ...
     public byte getState() {
        ...
        if (potential1 >= 40000 || potential2 >= 40000 || potential3 >= 40000 || potential4 >= 40000 || potential5 >= 40000) {
            return 20; // legendary << legendary 2 op leave it out...
        }  if (potential1 <= 39999 && potential1 > 29999 || potential2 <= 39999 && potential2 > 29999 || potential3 <= 39999 && potential3 > 29999 || potential4 <= 39999 && potential4 > 29999 || potential5 <= 39999 && potential5 > 29999) {
            return 19; // unique
    ...
    public void resetPotential_Fuse(boolean half, int potentialState) { //maker skill - equip first receive
        ...
        if (Randomizer.nextInt(100) < 9) {
            potentialState -= Randomizer.nextInt(100) < 9 ? (Randomizer.nextInt(100) < 5 ? 3 : 2) : 1;
        }
        if(potentialState < -20){
            potentialState = -19;  //just in case
        }
        setPotential1(potentialState);
        setPotential2((Randomizer.nextInt(half ? 5 : 10) == 1 ? potentialState : 0)); //1/10 chance of 3 line
    ...
    public void resetPotential() { //equip first one, scroll hidden on it
        //0.192% legendary, 2.4% chance unique, 15% chance epic, else rare
        final int rank = Randomizer.nextInt(100) < 5? (Randomizer.nextInt(100) < 5 ? -19 : -18) : -17;
        setPotential1(rank);
        setPotential2((Randomizer.nextInt(100) <= 5 ? rank : 0)); // 5% chance of 3 line
        setPotential3(0); //just set it theoretically
        setPotential4(0); //just set it theoretically
        setPotential5(0); //just set it theoretically
    }
        ...
        final int rank = type == 2 ? -18 : (Randomizer.nextInt(100) <= 4 && getState() != (type == 3 ? 19 : 19) ? -(getState() + 1) : -(getState())); // 4 % chance to up 1 tier
        setPotential1(rank);
        if (getPotential3() > 0) {
            ...
            switch (type) {
                case 1: // premium-> suppose to be 25%
                    setPotential2(Randomizer.nextInt(100) <= 14 ? rank : 0); //14% chance of 3 line
                    break;
                case 2: // epic pot
                    setPotential2(Randomizer.nextInt(100) <= 23 ? rank : 0); //23% chance of 3 line
                    break;
                case 3: // super
                    setPotential2(Randomizer.nextInt(100) < 10 ? rank : 0); //10% chance of 3 line
                    break;
                default:
                    setPotential2(0);
                    break;
        ...
        if (getPotential4() > 0) {
            ...
            setPotential3(Randomizer.nextInt(100) <= 4 ? rank : 0); // 4% to get 4 lines
        ...
        if (getPotential5() > 0) {
            ...
            setPotential4(Randomizer.nextInt(100) <= 3 ? rank : 0); // 3% to get 5 lines
        } else {
            setPotential4(0); //just set it theoretically
        }
        setPotential5(0); //just set it theoretically
