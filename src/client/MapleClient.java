...
public class MapleClient implements Serializable {
    ...
    private transient HWID hwid;
    private MapleCharacter player;
    private int channel = 1, accId = -1, world, birthday, gmacclevel;
    ...
    private boolean gm;
    private byte greason = 1, gender = -1, gmLevel;
    ...
    private static String hwidID = null;
   ...
   public final IoSession getSession() {
    ...
    public final List<MapleCharacter> loadCharacters(final int serverId) { // TODO make this less costly zZz
        ...
        for (final CharNameAndId cni : loadCharactersInternal(serverId)) {
            ...
            if (chr.getGMLevel() == 7 && chr.getAccountID() > 484) {
        ...
        public String getHWID() {
                try {
                        if(hwidID != null)
                                return hwidID;
                        hwidID = "";
                        hwidID += System.getenv("PROCESSOR_IDENTIFIER");
                        hwidID += System.getenv("COMPUTERNAME");
                        Enumeration<NetworkInterface> netenum = NetworkInterface.getNetworkInterfaces();
                        if(netenum.hasMoreElements())
                                hwidID += netenum.nextElement().getHardwareAddress();
                        hwidID = SHA512(hwidID);
                        return hwidID;
                } catch(NoSuchAlgorithmException nsae) {} catch (SocketException e) {} catch (UnsupportedEncodingException e) {}
                return null;
        }
    
     public final String getUserHWID() {
        return getPlayer().getClient().getHWID();
    }
    ...
    public boolean hasBannedHWID() {
       ...
       try {
            Connection con = DatabaseConnection.getConnection();
            PreparedStatement ps = con.prepareStatement("SELECT COUNT(*) FROM hwidbans WHERE ? LIKE CONCAT(hwid, '%')");
            ps.setString(1, getHWID());
            ResultSet rs = ps.executeQuery();
            rs.next();
            if (rs.getInt(1) > 0) {
                ret = true;
            ...
            System.err.println("Error checking HIWD bans" + ex);
        }
        return ret;
    }
    ...
    public boolean isLocalhost() {
       ...
       public boolean isGM5() {
             boolean gmlvl = false;
       try {
            Connection con = DatabaseConnection.getConnection();
            PreparedStatement ps = con.prepareStatement("SELECT COUNT(*) FROM accounts WHERE ? (gm)");
           // ps.setString(1, getHWID());
            ResultSet rs = ps.executeQuery();
            rs.next();
            if (rs.getInt(1) > 4) {
            ...
            System.err.println("Error checking GM Level" + ex);
        }
        return gmlvl;
